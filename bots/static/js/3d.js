...
console.log("change is reflected");
    ...
    function init() {
        ...
        document.getElementById("test").innerHTML =
            "<div class='row'><div class='col-sm-4'><input type='button' onclick='play()' value = 'Play' id = 'playbutton' class='gbut'>  </input></div>" + "<div class='col-sm-4'><input type='button' onclick='pause()' value = 'Pause' class='gbut'></input></div>"
            + "<div class='col-sm-4'><input type='button' onclick='nextmove()' value = 'Next' class='gbut'></input></div></div>";
	...
	for(count=1; count<21 ; count++)
	{
		if(  (initx[count])==cols)
		{
            ...
            if(iaa>10)
              {
							createPiece(iaa,piece,finalx,finaly,1 );
              }
            else
              {
                createPiece(iaa,piece,finalx,finaly,0);
              }
 							animate();
			    //}, 3000);
 							//break;
...
*/
	  ...
	  loader2.load("/static/models/board.js",function(geometry,materials){
    ...
    var line = new THREE.Mesh(new THREE.CubeGeometry(10, 1, 2,0),material_black);//add material after 1,1
...
*/
	...
	//scene.add( mesh2 );
	 ...
	 //load terrain
	  loader2.load("/static/models/terrain1.js",function(geometry,materials){
	  var material2= new THREE.MeshLambertMaterial(materials);
	  mesh2 = new THREE.Mesh(geometry, new THREE.MeshFaceMaterial(materials) );
	  mesh2.position.x=0;
	  mesh2.position.y=-2;
    ...
    var line = new THREE.Mesh(new THREE.CubeGeometry(112.5, 0.1, 0.1,0),material_black);//add material after 1,1

		        line.position.x =56;
		        line.position.z = p*6.25;
		        line.position.y = 0.1;
    ...
    var line = new THREE.Mesh(new THREE.CubeGeometry(0.6,0.1, 100,1),material_black);//add material after 1,1

		        line.position.x =p*6.25;
		        line.position.z = 56.25;
		        line.position.y = 0.1;
            line.rotation.z=Math.PI*1;
    ...
    function win(moves,index)
      {
                console.log("somebody won")
                document.getElementById("test1").innerHTML =
                            "<div class='row'><div class='winp'><h3>"+winner+" Won</h3></div></div>";
      }


    function erb(moves,index)
      {
        if(moves[index][1]=='1'){
        	if(moves[index][2]=='1'){
                document.getElementById("test1").innerHTML =
                            "<div class='row'><div class='winp'><h3>"+winner+" Won</h3><br><h4>"+loser+" diqualified due to exceeding time limit</h4><br><h5>This bot is slower than my Grandma!</h5></div></div>";
            	}
        	if(moves[index][2]=='2'){
                document.getElementById("test1").innerHTML =
                            "<div class='row'><div class='winp'><h3>"+winner+" Won</h3><br><h4>"+loser+" disqualified for forking</h4><br><h5>Team XOdia successfully defused the fork bomb.</h5></div></div>";
            	}
        	if(moves[index][2]=='3'){
                document.getElementById("test1").innerHTML =
                            "<div class='row'><div class='winp'><h3>"+winner+" Won</h3><br><h4>"+loser+" disqualified due to invalid move</h4><br><h5></h5></div></div>";
            	}
        	if(moves[index][2]=='4'){
                document.getElementById("test1").innerHTML =
                            "<div class='row'><div class='winp'><h3>"+winner+" Won</h3><br><h4>"+loser+" disqualified due to compilation error</h4><br><h5>Maybe you missed a semi-colon</h5></div></div>";
            	}
          	}
        else if(moves[index][1]=='2'){
        	if(moves[index][2]=='1'){
                document.getElementById("test1").innerHTML =
                            "<div class='row'><div class='winp'><h3>"+winner+" Won</h3><br><h4>"+loser+" diqualified due to exceeding time limit</h4><br><h5>This bot is slower than my Grandma!</h5></div></div>";
            	}
        ...
        {
          fun=wrapfunction(win,this,[moves,number]);
          callbacks.push(fun);
        }
      else if(h == 3)
        {
          fun=wrapfunction(erb,this,[moves,number]);
          callbacks.push(fun);
          break;
        }
